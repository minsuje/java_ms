...
// - 배열의 원소는 모두 같은 타입
// - 처음에 선언한 배열 크기 변경 불가능 ( 단, ArrayList 컬렉션의 경우, 동적으로 크기 조절 가능 )
public class ArrayEx {
    public static void main(String[] args) {
        ...
        // - 배열의 변수는 "참조 변수". 배열도 객체이므로 힙 영역에 생성되고, 배열 변수는 힘 영역의 주소를 저장
        // Null 저장 가능 ( null 로 초기화 )
        // ex. 타입[] 변수 = null;
        // => 배열 변수가 null 값을 가진 상태에서는 변수[인덱스]로 값을 읽거나 저장하게 되면 NullPointerException 발생
        String[] temp = null;
        System.out.println(temp);
        // System.out.println(temp[0]); // NullPointException
        ...
        System.out.println("intArray : " + intArray); // 저장된 참조값 출력
        ...
        // - new 연산자로 배열 선언과 생성
        // - new  연산자로 배열 처음 생성하면 기본 값으로 초기화 됨
        ...
        // 배열 길이
        // - 배열 변수.length
        // - length 필드는 읽기만, 값 변경 불가능
        System.out.println("doubleArray.length : " + doubleArray.length);
        // 배열 길이를 벗어난 다면?
//        System.out.println("doubleArray length : " + doubleArray[doubleArray.length]);
        ...
        for (int i = 0; i < threeDimensionArr.length; i++) {
            for (int j = 0; j < threeDimensionArr[i].length; j++){
                for (int k = 0; k < threeDimensionArr[i][j].length; k ++){
                    System.out.printf(threeDimensionArr[i][j][k] + " ");
                }
            }
            System.out.println();
        }
        ...
        // 배열 복사
        // - 배열은 크기 고정
        // -> 더 많은 저장 공간이 필요하다면 더 큰 길이의 배열을 새로 만들어 기존 배열을 복사
        ...
        // ver2. arraycopy() 사용
        // System.arraycopy(Object src, int srcPost, Object dest, int destPost, int length) => arraycopy()를 썼을 때 필요한 값들
        // - Object src: 원본 배열
        // - int srcPost : 원본 배열 복사 시작 인덱스
        // - Object dest : 새 배열
        // - int destPost: 새 배열의 붙여넣기 시작 할 인덱스
        // - int length : 복사 항목 수
        ...
        // copyOfRange(arr, sIdex, eIdex)
        int[] rangeArray = Arrays.copyOfRange(originalArray, 1,3); // [2, 3]
        System.out.println(Arrays.toString(rangeArray));

        // fill(arr, n)
        int[] fillArray = new int[5];
        System.out.println("filled Array (before) : " + Arrays.toString(fillArray)); // filled Array (before) : [0, 0, 0, 0, 0]
        Arrays.fill(fillArray, 7);
        System.out.println(Arrays.toString(fillArray)); // [7, 7, 7, 7, 7]

        // sort(err)
        int[] unsortedArray = {4,5,2,8,1,9,3};
        Arrays.sort(unsortedArray);
        System.out.println("unsortedArray : " + Arrays.toString(unsortedArray)); // [1, 2, 3, 4, 5, 8, 9]
        ...
        System.out.println("Equal : " + arraysEqual); // true
        System.out.println("Equal2 : " + arraysEqual2); // false

        // "==" 비교
        // 주소를 비교 하는 거라 false
        System.out.println("Equal : " + (array1 == array2)); // false
        System.out.println("Equal2 : " + (array1 == array3)); // false
        ...
        System.out.println("deep Equals : " + deepEqual2);

        // binarySearch(arr, val) 메소드 ( 단, 배열은 정렬된 상태여야 함 ) | 인덱스 찾기
        // 주의!, 미리 sort 하고 쓰면 좋다.
