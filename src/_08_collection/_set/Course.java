...
// - 동일한 객체 ( 동등 객체 )는 중복 저장하지 않음, 동일한 객체로 판단하는 기준이 존재
// - 다른 객체더라도 'hashCode()' 메소드의 리턴 값이 같고, 'equals()' 메소드가 'true' 를 리턴하면 동일한 객체로 판단

// TreeSet
// - 검색 기능을 강화시킨 컬렉션
// - 이진 트리를 기반으로 한 Set 컬렉션
// - 이진 트리 : 여러 개의 노드가 트리 형태로 연결된 구조로 root 노드라는 하나의 노드에서 시작해 각 노드의 최대 2개의 노드를 연결할 수 있다.
// 주의! 검색 관련 메소드는 TreeSet 에만 정의되어 있어 Set 타입 보다는 TreeSet 타입으로 선언하는 것이 좋음

/*
    Set<E> treeSet = new TreeSet<>(); ( X )
    TreeSet<E> treeSet = new TreeSey<>(); ( O )
 */
public class Course {
    private int id;
    ...
    // 참고! hashCode 란?
    // ㄴ hashCode: 객체의 메모리 주소를 기반으로 한 정수 값의 해시 코드
    // - hashCode() 메소드: 최상위 Object 클래스에 정의되어 있으며, 객체의 해시 코드를 반환하는 역할
